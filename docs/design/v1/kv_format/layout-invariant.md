...
The layout logic lives in `lmcache/v1/kv_format/`. The
...
- **Static facts are class attributes, declared once.** The structural shape
  ...
  `csrc/engine_kv_format.h`; `tests/v1/kv_format/test_kv_format_classification.py`
...
(while `contiguity.py` uses `union-attr` alone, and the `utils.py` facade keeps
the directive for its remaining structural helpers). The
