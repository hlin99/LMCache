...
Source: ``lmcache/v1/config.py``

PD mode enables disaggregated prefill-decode architecture where prefill and decode
operations run on separate nodes. The prefill node generates KV cache and transfers
it to the decode node via a shared buffer.
...
.. list-table::
   ...
   * - ``enable_pd``
     - ``False``
     - Enable PD (Prefill-Decode disaggregation) mode.
   * - ``pd_role``
     - ``None``
     - Role of the current node. Must be either ``"prefiller"`` or ``"receiver"``.
       Required when ``enable_pd`` is ``True``.
   * - ``pd_buffer_size``
     - ``None``
     - Size of the PD buffer in bytes. Required when ``enable_pd`` is ``True``.
   * - ``pd_buffer_device``
     - ``None``
     - Device for PD buffer allocation (e.g., ``"cuda:0"``, ``"cpu"``).
       Required when ``enable_pd`` is ``True``.
   * - ``pd_peer_host``
     - ``None``
     - Hostname or IP address of the peer node (prefiller or receiver).
   * - ``pd_peer_init_port``
     - ``None``
     - List of initialization ports for peer communication.
   * - ``pd_peer_alloc_port``
     - ``None``
     - List of allocation ports for peer communication.
   * - ``pd_proxy_host``
     - ``None``
     - Hostname or IP address of the proxy server (if used).
   * - ``pd_proxy_port``
     - ``None``
     - Port number of the proxy server (if used).
   * - ``pd_allocation_timeout_sec``
     - ``5.0``
     - Maximum seconds to retry memory allocation before giving up.
