    ...
    for object_idx, obj in enumerate(object_tensors):
        ...
        if is_d2h:
            ...
            for layer in layer_tensors:
                ...
                layer_all.append(layer_blocks.reshape(n_valid * block_size, layer.shape[-1]))
        ...
        if is_d2h:
            ...
            for layer_idx, layer in enumerate(layer_tensors):
                if int(gpu_kv_format) == int(GPUKVFormat.NL_X_TWO_NB_NH_BS_HS):
                    ...
                    k_t, v_t = layer[:, 0], layer[:, 1]
                _nb, nh, _bs, hs = k_t.shape
                k_blocks = chunk_gpu[0, layer_idx].reshape(n_valid, block_size, nh, hs).permute(
                    0, 2, 1, 3
                )
                v_blocks = chunk_gpu[1, layer_idx].reshape(n_valid, block_size, nh, hs).permute(
                    0, 2, 1, 3
                )
