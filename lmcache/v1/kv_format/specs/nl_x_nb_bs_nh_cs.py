...
"""Per-layer, NHD, blocks-first with fused K/V: ``NL x [NB, BS, NH, CS]``.
...
(``CS == 2 * head_size``, K/V packed). Like ``NL_X_NB_NH_BS_CS`` but tokens
before heads. Produced by vLLM's non-MLA blocks-first attention backends
(NHD layout).
"""
...
class NL_X_NB_BS_NH_CS_Probe(KVFormatProbe):
    engine_type = EngineType.VLLM
    format_spec = NL_X_NB_BS_NH_CS_Spec
    ...
    def probe(
        ...
        if (
            layout_hints.get("kv_layout") == "NHD"
