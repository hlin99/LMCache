...
"""Platform-abstraction base class for cache-context implementations.

:class:`BaseCacheContext` is the registry base class for all
device-specific cache-context backends. It subclasses :class:`abc.ABC`
...
Concrete backends live in device sub-packages such as
``lmcache.v1.platform.cuda`` and ``lmcache.v1.platform.cpu``. Each one
must define a non-empty ``device_type`` ClassVar so platform-agnostic
dispatch can resolve the right implementation for a given
``torch.device.type``.
"""
