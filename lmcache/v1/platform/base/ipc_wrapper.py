...
implementations. Every concrete wrapper (e.g.
:class:`~lmcache.v1.platform.cpu.shm.CpuShmTensorWrapper`,
:class:`~lmcache.v1.platform.cuda.ipc_wrapper.CudaIPCWrapper`) subclasses
it so they share the single msgspec ext code (1) -- pickle preserves the
concrete subclass identity across the wire so ``to_tensor`` dispatches
...
class DeviceIPCWrapper(abc.ABC):
