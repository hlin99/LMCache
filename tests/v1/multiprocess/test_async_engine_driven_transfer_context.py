...
def _new_context(
    ...
    *,
    ...
    ctx = AsyncEngineDrivenTransferContext(max_inflight_stores=max_inflight)
    ctx._engine_driven_context = _FakeStoreContext(commit_impl=commit_impl)
    return ctx
...
def test_sync_engine_driven_context_returns_resolved_future(
    ...
    ctx._engine_driven_context = _FakeStoreContext(commit_impl=lambda _c: True)
...
def test_build_engine_driven_context_dispatches_on_capability(
    ...
    monkeypatch.setattr(
        worker_transfer, "_supports_async_primitives", lambda _kv: True
    )
    ...
    capable = worker_transfer._build_engine_driven_context(kv_caches)
    ...
    fallback = worker_transfer._build_engine_driven_context(kv_caches)
