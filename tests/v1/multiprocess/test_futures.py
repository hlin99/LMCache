...
def _dev_ipc_available() -> bool:
    """Return True if the current device supports interprocess events."""
    return (
...
def _create_dev_event_in_process(event_queue: mp.Queue, delay: float = 0.0):
    """Helper process that creates a device event and sends the IPC handle."""
    # First Party
    from lmcache import torch_dev as _torch_dev

    _torch_dev.init()
    if delay > 0:
        time.sleep(delay)

    # Create and record a device event with interprocess flag
    event = _torch_dev.Event(interprocess=True)
    event.record()
...
@pytest.mark.skipif(
    not _dev_ipc_available(),
    reason="Device with IPC event support is required for CUDAMessagingFuture tests",
)
def test_cuda_messaging_future_basic_usage():
    ...
    process = ctx.Process(target=_create_dev_event_in_process, args=(event_queue,))
    process.start()
