...
def test_block_kv_transfer_python_fallback_matches_gather_scatter() -> None:
    """Facade python backend should preserve gather/scatter semantics."""
    # First Party
    ...
    for name in source:
        assert torch.allclose(dest_facade[name], dest_expected[name])


def test_block_kv_transfer_python_fallback_rejects_pointer_inputs() -> None:
    """Pointer-form arguments are not accepted by python fallback backend."""
    # First Party
    from lmcache.utils import EngineType
    from lmcache.v1.gpu_connector.utils import (
        get_block_size,
    ...
    paged_ptrs = torch.tensor(
        get_group_data_ptrs(normalized, fmt, [0]),
        dtype=torch.int64,
        device=tensors[0].device,
    )
    chunk = torch.empty(2, 1, block_size, 16)

    with pytest.raises(TypeError, match="tensor-form paged_buffer"):
        multi_layer_block_kv_transfer(
            paged_ptrs,
            [chunk],
            [0],
    ...
    with pytest.raises(TypeError, match="tensor-form lmcache_objects"):
        multi_layer_block_kv_transfer(
            normalized,
            [chunk.data_ptr()],
            [0],
