...
"""
Tests proving that the async PDBackendAsync sender and receiver execute
...
# ---------------------------------------------------------------------------
# Shared patch context for PDBackendAsync construction
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Fixture: PDBackendAsync sender with everything mocked out
# ---------------------------------------------------------------------------
...
def async_sender():
    """
    Build a PDBackendAsync in sender (prefiller) mode with:
...
# ---------------------------------------------------------------------------
# Fixture: PDBackendAsync receiver with allocator mocked out
# ---------------------------------------------------------------------------
...
def async_receiver():
    """
    Build a PDBackendAsync in receiver (decoder) mode.
...
def test_receiver_fail_fast_request_too_large(async_receiver):
    ...
    async def run():
        ...
        with pytest.raises(RuntimeError, match="max_inflight_chunks"):
            await async_receiver._async_allocate_and_put(_make_req(1, key_offset=MAX_T))
...
def test_receiver_admission_control_prevents_interleaving(async_receiver):
    ...
    assert a2_done_idx < b_done_idx, (
        f"req-A batch 2 should complete before req-B, but event_log={event_log}"
    )
...
def test_pd_max_prefill_len_capacity_check():
    """
    When pd_max_prefill_len > capacity_tokens, PDBackendAsync.__init__ must raise
    ...
    with p1, p2, p3, p4:
        with pytest.raises(ValueError, match="pd_max_prefill_len"):
            PDBackendAsync(_make_receiver_config(5000), metadata)

    # pd_max_prefill_len > capacity_tokens → ValueError for sender
    with p1, p2, p3, p4:
        with pytest.raises(ValueError, match="pd_max_prefill_len"):
            PDBackendAsync(_make_sender_config(5000), metadata)

    # pd_max_prefill_len == capacity_tokens → success (boundary: exactly fits)
    with p1, p2, p3, p4:
        backend = PDBackendAsync(_make_receiver_config(4096), metadata)
        backend.close()

    # pd_max_prefill_len = 0 → check skipped (default, legacy behaviour)
    with p1, p2, p3, p4:
        backend = PDBackendAsync(_make_receiver_config(0), metadata)
        backend.close()
...
def test_receiver_handle_alloc_request_sends_error_on_exception(async_receiver):
    ...
    assert isinstance(resp, AllocResponse), f"Expected AllocResponse, got {type(resp)}"
