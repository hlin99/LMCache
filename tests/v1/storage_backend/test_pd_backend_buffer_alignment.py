...
@patch.object(PDBackend, "_init_sender")
@patch(
...
def test_buffer_size_exact_alignment(
    _mock_ctx, _mock_channel, _mock_sender, _mock_receiver
):
    """
...
def test_buffer_size_alignment_sender_role(
    _mock_ctx, _mock_channel, _mock_sender, _mock_receiver
):
    """
    Test buffer size alignment with sender role.
    """
...
def test_buffer_size_too_small_raises_error(
    _mock_ctx, _mock_channel, _mock_sender, _mock_receiver
):
    """
    Test that a buffer size smaller than a single chunk raises ValueError.
    """

    metadata = create_test_metadata(kv_shape=(28, 2, 256, 8, 128))
    # Expected chunk size: 29360128 bytes

    config = LMCacheEngineConfig.from_defaults(
        chunk_size=256,
        pd_buffer_size=1000,  # Much smaller than chunk size
        pd_buffer_device="cpu",
        pd_role="receiver",
        pd_peer_host="localhost",
        pd_peer_init_port=[12345],
    ...
    with pytest.raises(ValueError, match="pd_buffer_size.*smaller than a single chunk"):
